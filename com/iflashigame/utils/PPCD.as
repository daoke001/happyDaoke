package com.iflashigame.utils
{
    ...
    public class PPCD
    {
		 ...
		 * @param	target1
		 * @param	target2
		 * @param	commonParent	两个对象所在的共同的容器
		 * @param	pixelPrecise	是否精确到像素。若设置为false，则和DisplayObject.hitTestObject()工作原理相同。
		 * @param	tolerance		容许量，默认为0。这个数值设置开始进行碰撞检测的DisplayObject的最小alpha值。
		 * 							比如有个对象有阴影效果，其阴影的alpha值大概在20以下。那么如果在进行碰撞检测
		 * 							的时候不希望把阴影也包含进去，则需要将tolerance设置为20。否则另外一个物体碰到阴影也算有碰撞。
		 * @return
		 */
